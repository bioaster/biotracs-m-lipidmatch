...
if(!require(installr)) {
   install.packages("installr", repo="https://cloud.r-project.org/"); 
   install.packages("stringr", repo="https://cloud.r-project.org/"); 
   require("installr");
}

if("sqldf" %in% rownames(installed.packages()) == FALSE) {install.packages("sqldf", repo="https://cloud.r-project.org/")}
if("compiler" %in% rownames(installed.packages()) == FALSE) {install.packages("compiler", repo="https://cloud.r-project.org/")}
if("gWidgets" %in% rownames(installed.packages()) == FALSE) {install.packages("gWidgets", repo="https://cloud.r-project.org/")}
if("gWidgetstcltk" %in% rownames(installed.packages()) == FALSE) {install.packages("gWidgetstcltk", repo="https://cloud.r-project.org/")}
if("optparse" %in% rownames(installed.packages()) == FALSE){install.packages("optparse", repo="https://cloud.r-project.org/")}
...
option_list = list(
  make_option(c("-r", "--RT_Window"), type="numeric", default=.2, 
              help="Retention Time plus or minus", metavar="numeric"),
  make_option(c("-p", "--ppm_Window"), type="numeric", default=10, 
              help="parts-per-million window for matching the m/z of fragments obtained in the library to those in experimentally obtained", metavar="numeric"),
  make_option(c("-m", "--PrecursorMassAccuracy"), type="numeric", default=0.005, 
              help="Tolerance for mass-to-charge matching at ms1 level (Window)", metavar="numeric"),
  make_option(c("-a", "--SelectionAccuracy"), type="numeric", default=0.4, 
              help="Plus minus range for the mass given after targeting parent ions portrayed in excalibur to match exact mass of Lipid in library", metavar="numeric"),
  make_option(c("-q", "--intensityCutOff"), type="numeric", default=1000, 
              help="Threshold for determining that the average signal intensity for a given MS/MS ion should be used for confirmation", metavar="numeric"),
  make_option(c("-d", "--CommentColumn"), type="numeric", default=1, 
              help="index of idColumn", metavar="numeric"),
  make_option(c("-z", "--MZColumn"), type="numeric", default=8, 
              help="index of MZColumn", metavar="numeric"),
  make_option(c("-t", "--RTColumn"), type="numeric", default=10, 
              help="index of RTColumn", metavar="numeric"),
  make_option(c("-w", "--RowStartForFeatureTableData"), type="numeric", default=2, 
              help="RowStartForFeatureTableData", metavar="numeric"),
  make_option(c("-s", "--ScanCutOff"), type="numeric", default=1, 
              help="The minimum number of scans required for the result to be a confirmation", metavar="numeric"),
  make_option(c("-i", "--InputDirectory"), type="character", default= "",
              help="Input Directory for Feature Table and MS2 files ...must have \\ (double backslash) at the end of the directory", metavar="character"),
  make_option(c("-n", "--InputDirectoryMs2Neg"), type="character", default= "",
              help="Input Directory for MS2 Neg files ...must have \\ (double backslash) at the end of the directory", metavar="character"),
  make_option(c("-k", "--InputDirectoryMs2Pos"), type="character", default= "",
              help="Input Directory for MS2 Pos files ...must have \\ (double backslash) at the end of the directory", metavar="character"),
  make_option(c("-o", "--OutputDirectory"), type="character", default= "",
              help="Output Directory for results ...must have \\ (double backslash) at the end of the directory", metavar="character"),
  make_option(c("-l", "--InputLibrary"), type="character",default= "",
              help=" Input Directory for Libraries ...must have \\ (double backslash) at the end of the directory", metavar="character")
); 
...
if (ManuallyInputVariables==TRUE){
  ...
  SelectionAccuracy<-opt$SelectionAccuracy
  
  #Threshold for determining that the average signal intensity for a given MS/MS ion should be used for confirmation
  intensityCutOff<-opt$intensityCutOff
  
  #Feature Table information
  CommentColumn <- opt$CommentColumn
  MZColumn <- opt$MZColumn
  RTColumn <- opt$RTColumn
  RowStartForFeatureTableData <- opt$RowStartForFeatureTableData #Look at your Feature Table (.csv)...What row do you first see numbers?
  
  
  #ddMS data? set this parameter. If not? Leave it.
  #The minimum number of scans required for the result to be a confirmation
  ScanCutOff<-opt$ScanCutOff
  ...
  InputDirectory_ms2Neg <-opt$InputDirectoryMs2Neg
  cat(paste0('Reading Input Library ', InputDirectory_ms2Neg, "\n"))
  
  InputDirectory_ms2Pos <-opt$InputDirectoryMs2Pos
  cat(paste0('Reading Input Library ', InputDirectory_ms2Pos, "\n"))
  
  OutputDir <- opt$OutputDirectory
  # Input Directory for Libraries ...must have \\ (double backslash) at the end of the directory
  InputLibrary<-opt$InputLibrary
  InputLibrary <- paste(InputLibrary, "//", sep="")
  cat(paste0('Reading Input Library ', InputLibrary, "\n"))
...
if(ManuallyInputVariables==TRUE || csvInput == TRUE){
  ...
  for (i in seq_len(length(foldersToRun))){
    ...
    if(length(foldersToRun)==0){#we're in current (and only) folder that contains feature table and ms2
      fpathMs2Neg <- InputDirectory_ms2Neg
      fpathMs2Pos <- InputDirectory_ms2Pos
      fpath<- InputDirectory
    }else if(foldersToRun[i] == "Output"){
      fpath <- InputDirectory
      print(paste("Warning: Remove your 'Output' folder from the current Input Directory:", InputDirectory))
...
CreateIDs <- function(PeakTableDirectory, ddMS2directory, Classdirectory, AIFdirectory, ImportLib, OutputDirectory, ddMS2, ddMS2Class, AIF, mode){
  ...
  cat(paste0(OutputDirectory, mode))
  # write.table(NewPeakTable, paste(OutputDirectory,mode,"IDed.csv",sep=""), sep=",",col.names=FALSE, row.names=FALSE, quote=TRUE, na="NA")
  write.table(NewPeakTable, paste(OutputDirectory,mode,"IDed.csv",sep=""), sep="\t",col.names=FALSE, row.names=FALSE, quote=FALSE, na="NA")
...
for(i in seq_len(lengthFoldersToRun)){
  ...
  fileNameMs2Neg <- basename(fpathMs2Neg)
  fileNameMs2Pos <- basename(fpathMs2Pos)
  
  ddMS2NEG_in <- list.files(path=fpathMs2Neg, pattern=".ms2", ignore.case=FALSE)
  AIFMS1NEG_in <- list.files(path=fpathMs2Neg, pattern="[nNgG]\\.ms1", ignore.case=FALSE)
  AIFMS2NEG_in <- list.files(path=fpathMs2Neg, pattern="[nNgG]\\.ms2", ignore.case=FALSE)
  
  ddMS2POS_in <- list.files(path=fpathMs2Pos, pattern=".ms2", ignore.case=FALSE)
  AIFMS1POS_in <- list.files(path=fpathMs2Pos, pattern="[pPsS]\\.ms1", ignore.case=FALSE)
  AIFMS2POS_in <- list.files(path=fpathMs2Pos, pattern="[pPsS]\\.ms2", ignore.case=FALSE)
  ...
  if(length(ddMS2POS_in) == 0){
    print(paste("CAUTION: We detected", length(ddMS2POS_in),"positive ddMS .ms2 files in the folder: ", fileNameMs2Pos," ...If this incorrect, check that you have 'p', 'P', 'pos', or 'POS' at the end of the file name and you must have a 'dd' within the name. OR Remove the folder: ", fileName))
  }
  if(length(ddMS2NEG_in) == 0){
    print(paste("CAUTION: We detected", length(ddMS2NEG_in),"negative ddMS .ms2 files in the folder: ", fileNameMs2Neg," ...If this incorrect, check that you have 'n', 'N', 'neg', or 'NEG' at the end of the file name and you must have a 'dd' within the name. OR Remove the folder: ", fileName))
  }
  ...
  if(length(foldersToRun)==0){#1 root folder
    cat(paste0(OutputDir,  "\n"))
    
    OutputDirectory<-paste(OutputDir, "\\Output\\", sep="")
    if(!dir.exists(OutputDirectory)){ dir.create(OutputDirectory) }
    if(runPosAIF || runNegAIF){#AIF
      OutputDirectoryAIF <- paste(OutputDir, "\\Output\\AIF\\", sep="")    
    ...
    if(runPosddMS || runNegddMS){#ddMS
      cat(paste0(OutputDir))
      OutputDirectoryddMS <- paste(OutputDir, "\\Output\\ddMS\\", sep="")    
      cat(paste0(OutputDirectoryddMS))
    ...
    cat(paste0(OutputDir, foldersToRun[i], "\n"))
    OutputDirectory <- paste(OutputDir, foldersToRun[i], "\\Output\\", sep="")
    cat(paste0(OutputDirectory, "\n"))
    if(!dir.exists(OutputDirectory)){ dir.create(OutputDirectory) }
    if(runPosAIF || runNegAIF){#AIF
  ...
  if(runNegddMS && nrow(LibraryCriteria)>0){
    ...
    for (c in 1:length(ddMS2NEG_in)){
      MS2_dir_in <- paste(fpathMs2Neg, ddMS2NEG_in[c], sep="")
  ...
  if(runPosddMS && nrow(LibraryCriteria)>0){
    ...
    for (c in 1:length(ddMS2POS_in)){
      MS2_dir_in <- paste(fpathMs2Pos, ddMS2POS_in[c],sep="")
  ...
  if(runNegddMS && nrow(LibraryCriteria)>0){
    ...
    for (c in 1:length(ddMS2NEG_in)){
      MS2_dir_in <- paste(fpathMs2Neg, ddMS2NEG_in[c], sep="")
      cat(paste0("  Reading in file:\t", fpathMs2Neg, ddMS2NEG_in[c],"\nFrom Directory:\t\t", MS2_dir_in,"\n"))
      ExtraSample<-ExtraSampleNameddMSNEG_in[c]
      MS2_df_in <- createddMS2dataFrame(MS2_dir_in)
      OutputInfo <- c(MS2_dir_in, ExtraSample, FeatureTable_dir_in)
  ...
  if(runPosAIF && nrow(LibraryCriteria)>0){
    ...
    for (c in 1:length(AIFMS1POS_in)){
      MS1_dir_in <- paste(fpathMs2Pos, AIFMS1POS_in[c], sep="")
      cat(paste0("Reading in file:\t", AIFMS1POS_in[c],"\nFrom Directory:\t\t", MS1_dir_in,"\n"))
      MS1_df_in <- createMS1dataFrame(MS1_dir_in)
      
      MS2_dir_in <- paste(fpathMs2Pos, AIFMS2POS_in[c], sep="")
