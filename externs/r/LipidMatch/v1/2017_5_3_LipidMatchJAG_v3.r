...
if (ManuallyInputVariables==TRUE){
  ...
  # InputDirectory<-"C:\\Users\\jabighanem\\LipidMatchShell\\"
  # 
  # # Input Directory for Libraries ...must have \\ (double backslash) at the end of the directory
  # InputLibrary<-"C:\\Users\\jabighanem\\Downloads\\2017_6_14_LipidMatch_Distribution\\LipidMatch_Libraries\\"
  ...
  # ####################### AUTO INPUTTING VARIABLES SECTION ###############################
  # 
  ...
  #   # numOfddFilesNeg <- length(list.files(path=fpathMs2Neg, pattern="[dD][dD]", ignore.case=FALSE))
  #   # numOfddFilesPos <- length(list.files(path=fpathMs2Pos, pattern="[dD][dD]", ignore.case=FALSE))
  #   numOfddFilesNeg <- length(list.files(path=fpathMs2Neg))
  #   numOfddFilesPos <- length(list.files(path=fpathMs2Pos))
  #   numOfddFiles <- numOfddFilesNeg + numOfddFilesPos
  #   
  #   numOfAIFFilesNeg <- length(list.files(path=fpathMs2Neg, pattern="[AIFaif][AIFaif][AIFaif]", ignore.case=FALSE))
  #   numOfAIFFilesPos <- length(list.files(path=fpathMs2Pos, pattern="[AIFaif][AIFaif][AIFaif]", ignore.case=FALSE))
  #   numOfAIFFiles <- numOfAIFFilesNeg + numOfAIFFilesPos
...
for(i in seq_len(lengthFoldersToRun)){
  ...
  AIFMS1NEG_in <- AIFMS1NEG_in[grep("[AIFaif][AIFaif][AIFaif]", AIFMS1NEG_in)]
  AIFMS2NEG_in <- AIFMS2NEG_in[grep("[AIFaif][AIFaif][AIFaif]", AIFMS2NEG_in)]
  
  # ddMS2POS_in <- ddMS2POS_in[grep("[dD][dD]", ddMS2POS_in)]

  AIFMS1POS_in <- AIFMS1POS_in[grep("[AIFaif][AIFaif][AIFaif]", AIFMS1POS_in)]
  AIFMS2POS_in <- AIFMS2POS_in[grep("[AIFaif][AIFaif][AIFaif]", AIFMS2POS_in)]
